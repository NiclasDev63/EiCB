...
public class CodeGenerator extends AstNodeBaseVisitor<Instruction, Void> {
	...
	public Instruction visitMatrixLhsIdentifier(MatrixLhsIdentifier matrixLhsIdentifier, Void __) {
		...
		// int totalSize = rowCount * colCount * elementWordSize;
		...
		// assembler.emitBoundsCheck(offset, rowCount);
		...
		// assembler.emitBoundsCheck(offset, colCount);
		...
		// return null;
		int offset = matrixLhsIdentifier.getDeclaration().getLocalBaseOffset();
        int wordSize = ((MatrixType) matrixLhsIdentifier.getDeclaration().getType()).elementType.wordSize;
        int rowCount = ((MatrixType) matrixLhsIdentifier.getDeclaration().getType()).rows;
        int colCount = ((MatrixType) matrixLhsIdentifier.getDeclaration().getType()).cols;

        int totalSize = rowCount * colCount * wordSize;

        // to compute the address of the selected index, we need to calculate offset + (rowIndex * colCount + colIndex) * wordSize
	...
	public Instruction visitVectorLhsIdentifier(VectorLhsIdentifier vectorLhsIdentifier, Void __) {
		...
		int elementWordSize = ((VectorType) vec@@torLhsIdentifier.getDeclaration().getType()).elementType.wordSize;
